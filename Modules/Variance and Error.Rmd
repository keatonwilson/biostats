---
title: "Centers, Variance and Error - Week 6"
...
```
##Learning Objectives:  
  1. Students will be able to describe the differences between the different measures of center for sets of data (mean, median, mode), and be able to deploy them to summarize data sets. 
  
  2. Students will be able to describe the utility of attributing error/variance to data sets (real-world scenarios) - why error and variance matters (more lying with charts).
  
  3. Students will be able to describe different ways of attributing error to data sets (SE, SD, CI, variance, IQR) - REAL definition of 95% confidence interval (subtle, but important).
  
  4. Students can use variance and error to make interesting inferences about data (outliers - maybe simpler is better here).  

#### 1a. Why are summary statistics important?  
...
#### 1c. Measures of center
Scientists use measures of center to describe the overal properties of a group, but also to make comparisons among groups.  

You'll see sentences in results sections of papers like: "Flies that were fed a reduced-protein diet had average wing sizes that were smaller (3.2 mm plus or minues 0.2 mm) than flies that were fed a control diet (6.7 mm plus or minues 0.35 mm)."  

There are three main ways to calculate the "center" of a distribution of data, the mean, median and mode.  

What are they?

Mean = sum everything, divide the number of things you summed.
Median = the most middle number... (the middlest?), if there are two, average the two in the middle.
Mode = the most common (this is used a lot less, and there actually isn't a built-in function in R for it).
R has built in functions that do this for us.  
...
#Why don't we calculate the centers of a few of these columns? What is the mean and median of bodywt?
...
We'll have an example later of instances where it's best to use median versus mode, but overall the rule is:  
  1. Use mean if you have a big sample size that is normally distributed. 
  2. Use median if you have a smaller sample size that is skewed.

#### 1d. Group Challenge
We calculated means of columns above, but that's not really useful, what is more informative would be calculating the average brain weights for all the different orders of mammals. Can you accomplish this?
Work together in your group to write a script that does this. 

```{r}
#There will be a variety of ways to do this, but the easiest is with the pipe and dyplyr
library(tidyverse)

msleep %>%
  group_by(order) %>%
  filter(!is.na(brainwt)) %>%
  dplyr::summarize(avg_brainwt = mean(brainwt, na.rm = TRUE),
                   n())
...
Let's take what we've learned and build some charts in R. We talked a few weeks ago about data visualization, and everyone has seen typcial bar charts - be it in scientific papers or on Fox News. Let's plot some stuff and explore what measures of center we might use, and why visualizing error is important.
...
#### 1f. Group Challenge #2 - What's wrong with this plot?
